...
## Process for Creating State Boundary Shapefile for Midwestern States only
...
##### Maximum Temperature
# Increase space at top of map - create new bounding box
...
######################################################################
####### Map of Midwestern FIA Plot Locations

## I just created this for the presentation - it takes a very long time to run because it is tons of data, so I have commented it out here
...
######################################################################
######################################################################
### Variation in Soil Environment (Soil Organic Carbon and Available Water Storage) for Species

## Function used to aggregate resolution of soil rasters from 500 x 500 m (resolution when the TIFs were created in ArcGIS) to the 1000 x 1000 m scale
# myfun <- function(x){
...
## Read in resampled (1000 x 1000 m) soil organic carbon (SOC) and soil water availability (soilW) rasters for each state - files labeled as 500m but are all 1000 x 1000 m
...
#################################################################################
### For each state: 
### 1) Make average plot coordinates (lat/long) into spatial features for each state
### 2) Group data by Plot, Species Codes (SPCD), and Month to only include one tree per species per plot per month (reduce dataset size - i.e., create presence/absence by spp)
### 3) Use the extract function to create a new column with the Soil Organic Carbon and Available Soil Water for each plot
### 4) Group by species/plot and 
...
##### Min Available Water Storage - Excluding IA b/c data is not across the same soil depth (so values are not comparable)
...
##### Max Available Water Storage - Excluding IA b/c data is not across the same soil depth (so values are not comparable)
...
#######################################################################
#######################################################################
### Filter to determine minimum/maximum SOC and AWS per species across Midwest, and plot these data
